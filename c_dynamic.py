...
class dynamic_rules(app_manager.RyuApp):
    ...
    def long_path(self, src, dst, bw=0):
        ...
        distance = defaultdict(lambda:None) #distance[i]the longest path start from i
        result = defaultdict(lambda:defaultdict(lambda:None))
        vis = {}
        ...
        while len(seen) < len(self.src_links):
            ...
            for(temp_src, temp_dst) in self.src_links[node]:
                if temp_dst not in seen:
                    seen.append(temp_dst)
                    vis[temp_dst] = 0
                    distance[temp_dst] = -100
        node = dst
        distance[dst]=0
        i = 0
        les = len(seen)
        #self.logger.info("seen:")
        #print(seen)
        while 1:
            while seen is not None:
                if vis[node] == 1:
                    ...
                    for(temp_src, temp_dst) in self.src_links[node]:
                        temp_src_port = self.src_links[node][(temp_src, temp_dst)][0]
                        temp_dst_port = self.src_links[node][(temp_src, temp_dst)][1]
                        #self.logger.info("temp_src:%d, temp_dst:%d", temp_src,temp_dst)
                        if (distance[node] + w > distance[temp_dst]) and (temp_dst in seen):
                            distance[temp_dst] = distance[node] + w
                            result[node] = (temp_src, temp_src_port, temp_dst, temp_dst_port)
                        if(les == 1):
                            if distance[temp_dst] > temp:
                                temp = distance[temp_dst]
                                result[node] = (temp_src, temp_src_port, temp_dst, temp_dst_port)
                    seen.remove(node)
                    les = les - 1
            if les > 0:
                ...
                break
        path = []
        #self.logger.info("do,src:%d, dst:%d", src, dst)
        #print(distance[src])
        #print(result)
        if src == 1:
            result[dst] = (5,2,3,2)
    ...
    def delete_flow(self, datapath, priority, match, actions, idle_timeout=10, hard_timeout=60):
        ...
        actions = []
        inst = []
        req = ofp_parser.OFPFlowMod(datapath,cookie, cookie_mask, table_id,ofp.OFPFC_DELETE, idle_timeout,hard_timeout,priority, buffer_id,ofp.OFPP_ANY, ofp.OFPG_ANY, ofp.OFPFF_SEND_FLOW_REM,match, inst)
    ...
    def get_OFPPortStatus_msg(self, ev):
        ...
        if self.path != None:
            ...
            for i in range(len(self.path) - 2, -1, -2):
                datapath_path = self.datapaths[self.path[i][0]]
                match1 = parser.OFPMatch(in_port=self.path[i][1], eth_src=src, eth_dst=dst, eth_type=0x0800,ipv4_src=srcip, ipv4_dst=dstip)
                self.delete_flow(datapath=datapath_path,priority = 100, match=match1, actions = None, idle_timeout = 10, hard_timeout = 60)
                match2 = parser.OFPMatch(in_port=self.path[i+1][1], eth_src=dst, eth_dst=src, eth_type=0x0800,ipv4_src=dstip, ipv4_dst=srcip)
                self.delete_flow(datapath=datapath_path,priority = 100, match=match2, actions = None, idle_timeout = 10, hard_timeout = 60)
